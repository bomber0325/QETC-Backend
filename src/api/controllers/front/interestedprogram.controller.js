...
exports.edit = async (req, res, next) => {
  try {
    ...
    return res.send({
      success: true,
      message: "interestedprogram updated successfully",
      interestedprogram,
    });
  } catch (error) {
    return next(error);
  ...
  try {
    ...
    if (id) {
      ...
      if (interestedprogram)
        return res.send({
          success: true,
          message: "interestedprogram Page deleted successfully",
          id,
        });
      else
        return res.status(400).send({
          success: false,
          message: "interestedprogram Page not found for given Id",
        });
    } else
      return res
        .status(400)
